...
JOIN fakultas f ON f.id_fakultas = ps.id_fakultas
JOIN perguruan_tinggi pt ON pt.id_perguruan_tinggi = f.id_perguruan_tinggi
WHERE pt.npsn = '@npsn' AND ms.id_semester = '@id_semester' AND ms.fd_id_smt IS NOT NULL AND ms.fd_id_reg_pd IS NOT NULL AND ms.fd_sync_on < ms.updated_on
UNION ALL
/* Jumlah Bakal Insert */
SELECT count(*) AS jumlah FROM mahasiswa_status ms
JOIN mahasiswa m ON m.id_mhs = ms.id_mhs
...
JOIN fakultas f ON f.id_fakultas = ps.id_fakultas
JOIN perguruan_tinggi pt ON pt.id_perguruan_tinggi = f.id_perguruan_tinggi
WHERE pt.npsn = '@npsn' AND ms.id_semester = '@id_semester' AND ms.fd_id_smt IS NULL AND ms.fd_id_reg_pd IS NULL