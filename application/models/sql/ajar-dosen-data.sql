...
WHERE pt.npsn = '@npsn' AND ps.kode_program_studi = '@kode_prodi' AND pjmk.pjmk_pengampu_mk = '1'
UNION ALL
/* Jumlah sudah link */
SELECT count(*) AS jumlah FROM pengampu_mk pjmk
JOIN kelas_mk kls ON kls.id_kelas_mk = pjmk.id_kelas_mk
...
JOIN fakultas f ON f.id_fakultas = ps.id_fakultas
JOIN perguruan_tinggi pt ON pt.id_perguruan_tinggi = f.id_perguruan_tinggi
WHERE pt.npsn = '@npsn' AND ps.kode_program_studi = '@kode_prodi' AND pjmk.pjmk_pengampu_mk = '1' AND 
